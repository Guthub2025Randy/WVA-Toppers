...
P_B0 = 650000 #[W] Nominal electric engine  power from Zuidgeest # (Aanpassing van vout volgens Brightspace Announcement)
M_B0 = P_B0 / (2 * math.pi * n_e0) #([P_b*1000/2/math.pi/n_eng_nom])
...
eta_hull = (1-thrust_factor)/(1-w) #P_E_out/(P_T_out*k_p) # (Aanpassing na Brightspace announcement volgens Stapersma page 70)
eta_hull_out = np.linspace(eta_hull, eta_hull, len(t_out)) #P_E_out/(P_T_out*k_p) # (Aanpassing na Brightspace announcement volgens Stapersma page 70)

# WVA1 OPDRACHT 4 # #############################################################################################################
W_loss_mech = 296.29 + 691.38*(n_e_out/n_e0)
Q_loss_cool = 795.33 + 3181.333*(P_B_out/P_B0)
Q_loss_comb = 0.0 # Assume no heat loss in Combustion
eta_q = 1.0 # Due to no heat loss in Combustion
eta_th = 0.52 # Thermodynamic efficiency
W_e = P_B_out*t_out
eta_m = W_e/W_loss_mech
Q_in = W_loss_mech/eta_th
Q_comb = Q_in/eta_q
eta_comb = Q_comb/Q_f_out

eta_total = eta_m*eta_comb*eta_q*eta_th
print("The total efficiency = ",eta_total)
###################################################################################################################################
