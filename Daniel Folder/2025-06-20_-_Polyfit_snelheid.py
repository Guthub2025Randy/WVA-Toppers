...
20250620: Daniel: plots en snelheid
"""
...
def calcPolyfit(x_array, a, b, c):
    y_values = a*x_array**2 + b*x_array + c
    return y_values
...
########################################################################################
times = sol.t
a_vs, b_vs, c_vs = np.polyfit(times, v_s, 2)
t = np.linspace(0, Total_time, 1000)
v_s_fitted = calcPolyfit(t, a_vs, b_vs, c_vs)

plt.plot(t, v_s_fitted)
plt.grid(True)
plt.show()
########################################################################################
...
plt.xlabel('vermogen [watt]')
plt.ylabel('efficiëntie')
plt.grid()
plt.title('de verschillende efficiënties')
plt.show()

plt.plot((v_s*3.6)/1.852, R_sp)
plt.xlabel('snelheid [kts]')
...
#plt.ylim(0, 16)
plt.plot(times, n_EM, times, n_p, linewidth=width)
plt.legend(('Engine rotation speed [rps]', 'Propeller rotation speed [rps]'), loc='upper right')
plt.xlabel('Time [s]')
plt.ylabel('Engine speed $n_e$ [rps] / Propeller speed $n_p$ [rps]')
plt.grid(True)


plt.figure(3, figsize=(16,9))
plt.title('figuur 3', loc='right', pad=20)
plt.plot(v_s*(3600/1852), R/1000, linewidth=width)
#plt.xlim([0, 10])
#plt.ylim([0, 250])
...
plt.plot(v_a * 3600 / 1852, F_prop, linewidth=width)
#plt.xlim([0, 10])
plt.xlabel('Nominal ship Speed $v_a$ (with wake-factor from H&M considered) [knots]')
plt.ylabel('Delivered Thrust T [N]')
plt.grid(True)


plt.figure(5, figsize=(16,9))
plt.title('figuur 5', loc='right', pad=20)
plt.plot(n_p * 60, M_prop / 1000, linewidth=width)
#plt.xlim([0, 150])
#plt.ylim([0, 90])
...
plt.plot(n_EM * 60, M_EM, 'b',  linewidth=width)
#plt.xlim([0, 1000])
#plt.ylim([0, 18000])
...
plt.plot(times, P_Eff/1000, 'b')
plt.xlabel('travel time t [s]')
plt.ylabel('Effectife Power $P_E$ [kW]')
plt.grid(True)
plt.subplot(4,1,2)
plt.plot(times, P_D/1000, 'g')
plt.xlabel('travel time t [s]')
plt.ylabel('Delivered Power $P_D$ [kW]')
plt.grid(True)
plt.subplot(4,1,3)
plt.plot(times, P_B/1000, 'r')
plt.xlabel('travel time t [s]')
plt.ylabel('Engine power $P_B$ [kW]')
plt.grid(True)
plt.subplot(4,1,4)
plt.plot(times, Q/1000, 'k')
plt.xlabel('travel time t [s]')
plt.ylabel('Moment Q [kNm]')
plt.grid(True)
plt.tight_layout()
plt.show()


plt.figure(8, figsize=(16,16))
plt.subplot(4,1,1)
plt.title('figuur 8', loc='right', pad=20)
plt.plot(times, eta_hull, 'b')
plt.xlabel('travel time t [s]')
plt.ylabel(r'$\eta_{hull}$ [-]')
plt.grid(True)
plt.subplot(4,1,2)
plt.plot(times, eta_O, 'g')
plt.xlabel('travel time t [s]')
plt.ylabel(r'$\eta_O$ [-]')
plt.grid(True)
plt.subplot(4,1,3)
plt.plot(times, np.linspace(eta_TRM, eta_TRM, len(times)), 'r')
plt.xlabel('travel time t [s]')
plt.ylabel(r'$\eta_{TRM}$ [-]')
plt.grid(True)
plt.subplot(4,1,4)
plt.plot(times, eta_EM, 'k')
plt.xlabel('travel time t [s]')
plt.ylabel(r'$\eta_e$ [-]')
plt.grid(True)
plt.tight_layout()
...
plt.plot( P_P / 1000, eta_O, color='blue', label=r'$\eta_O$')
plt.plot( P_B / 1000, eta_EM, color='green', label=r'$\eta_e$', linewidth=width)
